...
static constexpr uint32_t BAUD_RATE_PREF_HASH = 0x5379609B;
static constexpr uint32_t BAUD_PROBE_SWITCH_DELAY_MS = 30;
static constexpr uint32_t BAUD_PROBE_TIMEOUT_MS = 80;
static constexpr std::array<uint32_t, 6> BAUD_RATE_CANDIDATES = {9600, 4800, 19200, 38400, 115200, 2400};

void SY7T609_UART::setup()
{
  this->configured_baud_rate_ = this->parent_ != nullptr ? this->parent_->get_baud_rate() : 0;
  if (global_preferences != nullptr)
  {
    this->baud_rate_pref_ = global_preferences->make_preference<uint32_t>(BAUD_RATE_PREF_HASH, true);
    if (!this->baud_rate_pref_.load(&this->stored_baud_rate_)) {
      this->stored_baud_rate_ = 0;
    }
  }
  this->clear_uart_buffer_();
  this->baud_locked_ = this->ensure_baud_rate_locked_();
}
...
void SY7T609_UART::loop() 
{
  ...
  if (now - this->last_read_ > 20 && this->available() == SSI_UART_READ_RECV_PKG_SIZE) 
  {
    ...
    bool bSuccess = this->validate_response_(resp, true);
...
{
  if (!this->baud_locked_ && !this->ensure_baud_rate_locked_())
  {
    ESP_LOGW(TAG, "SY7T609_UART skip update because baud rate is not detected yet");
    return;
  }
...
void SY7T609_UART::clear_uart_buffer_()
{
...
bool SY7T609_UART::validate_response_(const std::array<uint8_t,SSI_UART_READ_RECV_PKG_SIZE> &array, bool log_invalid)
{
  if (array[0] != REPLY_ACK_WITH_DATA)
  {
    if (log_invalid)
    {
      ESP_LOGW(TAG, "SY7T609_UART invalid REPLY_ACK_WITH_DATA! 0x%02X", array[0]);
    }
    return false;
  }

  uint8_t sum = checksum<SSI_UART_READ_RECV_PKG_SIZE>(array);
  if (sum != array[array.max_size() - 1])
  {
    if (log_invalid)
    {
      ESP_LOGW(TAG, "SY7T609_UART invalid checksum! 0x%02X != 0x%02X", sum, array[array.max_size() - 1]);
    }
    return false;
  }
...
void SY7T609_UART::apply_baud_rate_(uint32_t baud_rate)
{
...
void SY7T609_UART::save_detected_baud_rate_(uint32_t baud_rate)
{
  if (baud_rate == 0 || baud_rate == this->stored_baud_rate_)
  {
    return;
  }

  this->stored_baud_rate_ = baud_rate;
  if (this->baud_rate_pref_.save(&this->stored_baud_rate_) && global_preferences != nullptr)
  {
    global_preferences->sync();
  }
}

bool SY7T609_UART::try_probe_baud_rate_(uint32_t baud_rate)
{
  if (baud_rate == 0)
  {
    return false;
  ...
  this->apply_baud_rate_(baud_rate);
  delay(BAUD_PROBE_SWITCH_DELAY_MS);
  this->clear_uart_buffer_();
  this->uartSendReadCmd(ADDR_PF);

  const uint32_t started = millis();
  while (millis() - started < BAUD_PROBE_TIMEOUT_MS)
  {
    if (this->available() >= SSI_UART_READ_RECV_PKG_SIZE)
    {
      auto resp = *this->read_array<SSI_UART_READ_RECV_PKG_SIZE>();
      this->clear_uart_buffer_();
      if (!this->validate_response_(resp, false))
      {
        return false;
      }

      this->detected_baud_rate_ = baud_rate;
      this->baud_locked_ = true;
      this->save_detected_baud_rate_(baud_rate);
      ESP_LOGI(TAG, "SY7T609_UART locked baud rate: %" PRIu32, baud_rate);
      return true;
    }
    delay(1);
  }
...
bool SY7T609_UART::ensure_baud_rate_locked_()
{
  if (this->baud_locked_)
  {
    return true;
  }

  if (this->stored_baud_rate_ != 0 && this->try_probe_baud_rate_(this->stored_baud_rate_))
  {
    return true;
  }

  if (this->configured_baud_rate_ != 0 && this->configured_baud_rate_ != this->stored_baud_rate_ &&
      this->try_probe_baud_rate_(this->configured_baud_rate_))
  {
    return true;
  }

  for (const uint32_t baud_rate : BAUD_RATE_CANDIDATES)
  {
    if (baud_rate == this->stored_baud_rate_ || baud_rate == this->configured_baud_rate_)
    {
      continue;
    }
    if (this->try_probe_baud_rate_(baud_rate))
    {
      return true;
    }
  }

  if (this->stored_baud_rate_ != 0)
  {
    this->apply_baud_rate_(this->stored_baud_rate_);
  }
  else if (this->configured_baud_rate_ != 0)
  {
    this->apply_baud_rate_(this->configured_baud_rate_);
  }

  ESP_LOGW(TAG, "SY7T609_UART baud auto-detect failed");
  return false;
}
...
void SY7T609_UART::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Configured UART Baud: %" PRIu32, this->configured_baud_rate_);
  if (this->stored_baud_rate_ != 0)
  {
    ESP_LOGCONFIG(TAG, "  Stored UART Baud: %" PRIu32, this->stored_baud_rate_);
  }
  if (this->detected_baud_rate_ != 0)
  {
    ESP_LOGCONFIG(TAG, "  Active UART Baud: %" PRIu32, this->detected_baud_rate_);
  }
  else
  {
    ESP_LOGW(TAG, "  Active UART Baud: not detected yet");
  }
...
void SY7T609_UART::handleActionCallback()
{
  ...
  if (!this->baud_locked_ && !this->ensure_baud_rate_locked_())
  {
    ESP_LOGW(TAG, "SY7T609_UART skip callback because baud rate is not detected yet");
    return;
  }

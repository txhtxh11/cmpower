...
class SY7T609_UART : public PollingComponent, public uart::UARTDevice 
{
 ...
 protected:
  ...
  bool validate_response_(const std::array<uint8_t,SSI_UART_READ_RECV_PKG_SIZE> &array, bool log_invalid);

  void apply_baud_rate_(uint32_t baud_rate);

  bool try_probe_baud_rate_(uint32_t baud_rate);

  bool ensure_baud_rate_locked_();

  void save_detected_baud_rate_(uint32_t baud_rate);
...
protected: 
  ...
  uint32_t stored_baud_rate_{0};
  uint32_t detected_baud_rate_{0};
...
private:
  ...
  ESPPreferenceObject baud_rate_pref_{};
